...
class MorganFingerprintEmbedding(torch.nn.Module):
    def __init__(self, hidden_dim: int, fingerprint_path: str, nbits: int = 2048):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.nbits = nbits
        self.embedding = torch.nn.Linear(in_features=nbits, out_features=hidden_dim)

        fingerprint_path = str(Path(fingerprint_path).expanduser())
        mol_ids: list[int] = []
        onbits: dict[int, list[int]] = {}
        with open(fingerprint_path) as f:
            for line in f:
                row = json.loads(line)
                mol_id = int(row["mol_id"])
                mol_ids.append(mol_id)
                onbits[mol_id] = [int(bit) for bit in row.get("morgan_onbits", [])]

        max_mol_id = max(mol_ids) if mol_ids else -1
        fingerprints = torch.zeros(max_mol_id + 1, nbits, dtype=torch.float32)
        for mol_id, bits in onbits.items():
            valid_bits = [bit for bit in bits if 0 <= bit < nbits]
            if valid_bits:
                fingerprints[mol_id, valid_bits] = 1.0

        self.register_buffer("fingerprints", fingerprints, persistent=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        mol_ids = x.long().reshape(-1).to(device=self.fingerprints.device)
        fp = self.fingerprints[mol_ids]
        return self.embedding(fp.to(device=self.device))
