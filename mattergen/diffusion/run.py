...
def main(
    ...
    warmstart_checkpoint_path = config.get("warmstart_checkpoint_path", None)
    if ckpt_path and warmstart_checkpoint_path:
        raise AmbiguousConfig(
            f"Ambiguous config: set either checkpoint_path={ckpt_path} for trainer resume "
            f"or warmstart_checkpoint_path={warmstart_checkpoint_path} for weight-only init, not both."
        )
    ...
    if warmstart_checkpoint_path:
        checkpoint = torch.load(warmstart_checkpoint_path, map_location="cpu")
        ...
        logger.info(
            "Warm-started model weights from %s: loaded %d/%d checkpoint tensors. "
            "Optimizer, scheduler, and trainer loop state are not restored.",
